...
const getPrices = async (req, res) => {
  try {
    ...
    if (crop) {
      query += ` AND LOWER(crop_name) = LOWER($${paramIdx++})`;
      params.push(crop);
    }
    if (state) {
      query += ` AND LOWER(state) = LOWER($${paramIdx++})`;
      params.push(state);
    }
    if (city) {
    ...
    // Group by crop for better frontend display
    const grouped = {};
...
const calculateProfit = async (req, res) => {
  try {
    const { crop, quantity, transport_costs, home_city } = req.body;
    ...
    // Get all market prices for this crop
    ...
    const calculations = result.rows.map(row => {
      ...
      // Get transport cost for this city (if provided)
      let transportCost = 0;
      if (transport_costs && transport_costs[row.city] !== undefined) {
        transportCost = parseFloat(transport_costs[row.city]);
      } else if (transport_costs && transport_costs.default !== undefined) {
        transportCost = parseFloat(transport_costs.default);
      }

      const profit = totalRevenue - transportCost;
      const profitPerKg = profit / qty;
      ...
      return {
        ...
        net_profit: profit,
        profit_per_kg: profitPerKg.toFixed(2),
    ...
    if (bestOption.net_profit > 0) {
      recommendation = `🎯 Best market: Sell in ${bestOption.city} (${bestOption.state}) at ₹${bestOption.price_per_kg}/kg. Net profit: ₹${bestOption.net_profit.toFixed(0)}`;
    } else {
      recommendation = `⚠️ All options show loss with current transport costs. Consider reducing quantity per trip or finding local buyers.`;
    }
    ...
    res.json({
      ...
      summary: {
        ...
        min_profit: worstOption.net_profit
      }
    });
  } catch (err) {
